...
//!
//! env：`GROK2API_SIGNER_MODE=remote|local|fake`（缺省 remote）。
...
impl std::str::FromStr for SignerMode {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.trim().to_ascii_lowercase().as_str() {
            ...
            other => Err(format!(
                "GROK2API_SIGNER_MODE 非法: {other}（期望 remote|local|fake）"
            )),
        }
    }
}
...
mod tests {
    ...
    fn mode_parse_roundtrip() {
        ...
        assert_eq!("".parse::<SignerMode>().unwrap(), SignerMode::Remote);
