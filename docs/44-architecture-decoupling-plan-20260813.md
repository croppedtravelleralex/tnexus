...
### A.3 改造：增量对账（reconcile），不是整池重载 —— **已实现**

> 状态：已落地于 `SimplifiedPool::reconcile`（`crates/grok-pool/src/lib.rs`）
> + 后台任务 `grok_pool_reconcile`（`crates/grok2api-rs/src/tasks.rs`，间隔由
> `GROK_POOL_RECONCILE_INTERVAL_MS` 控制，默认 60s）。**尚未部署**。
...
| 序 | 动作 | 工作量 | 收益 |
|----|------|--------|------|
| ~~P0-1~~ | ~~Grok 号池增量对账 + PG 冷却合并~~ **已实现，待部署** | 中 | 管理台操作即时生效；不再靠重启；消除 A 类"两个平行宇宙" |
